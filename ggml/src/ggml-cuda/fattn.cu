...
void ggml_cuda_flash_attn_ext(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Supports all turbo types with matched K/V. Decode only (Q->ne[1] <= 4).
    // Set GGML_TURBO_MMA_FUSED=0 to disable.
    ...
    if (turbo_mma_fused && turbo_matched && Q->ne[1] <= 4 &&
        ...
        // TCQ types: load codebooks and alpha before dispatch
        if (K->type == GGML_TYPE_TURBO3_TCQ || K->type == GGML_TYPE_TURBO2_TCQ) {
            ...
            if (d_tcq_decode_alpha_k > 0.0f) {
                cudaMemcpyToSymbol(d_tcq_decode_alpha_k_fattn, &d_tcq_decode_alpha_k, sizeof(float));
            }
            // Load runtime codebook overrides (same logic as VEC path, guarded by static once-flags)
            if (K->type == GGML_TYPE_TURBO3_TCQ) {
                static bool tcq3_fused_loaded[GGML_CUDA_MAX_DEVICES] = {};
                if (!tcq3_fused_loaded[ctx.device]) {
                    tcq3_fused_loaded[ctx.device] = true;
                    const char *cb_path = getenv("TURBO_TCQ_CB");
                    if (cb_path) {
                        float cb[512];
                        FILE *f = fopen(cb_path, "rb");
            ...
            if (K->type == GGML_TYPE_TURBO2_TCQ) {
                static bool tcq2_fused_loaded[GGML_CUDA_MAX_DEVICES] = {};
                if (!tcq2_fused_loaded[ctx.device]) {
                    tcq2_fused_loaded[ctx.device] = true;
                    const char *cb_path = getenv("TURBO_TCQ_CB2");
                    if (cb_path) {
                        float cb[256];
                        FILE *f = fopen(cb_path, "rb");
        ...
        // Pre-rotate Q: turbo K stays in WHT-rotated domain, so Q must be rotated to match.

...
struct common_sampler * common_sampler_init(const struct llama_model * model, struct common_params_sampling & params) {
    ...
    // reasoning budget sampler — always created when thinking tags are present so that
    // EOS is suppressed inside <think> blocks (prevents malformed responses with MTP)
